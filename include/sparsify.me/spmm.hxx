...
float spmm(ell_t<type_t, util::memory_space_t::device>* As,
  ...
  for (std::size_t batch = 0; batch < batch_size; ++batch) {
    ...
    cusparseCreateBlockedEll(&desc_A,
                              A.rows, A.cols, A.block_size,
                              A.ell_cols, A.column_indices.data().get(), 
                              A.values.data().get(),
                              CUSPARSE_INDEX_32I,
                              CUSPARSE_INDEX_BASE_ZERO, CUDA_R_16F);
    
    // Create dense matrix C
    cusparseCreateDnMat(&desc_C, m, n, m, C,
                          CUDA_R_16F, CUSPARSE_ORDER_COL);
  }

  // Create dense matrix B
  cusparseCreateDnMat(&desc_B, k, n, k, B,
                        CUDA_R_16F, CUSPARSE_ORDER_COL);
